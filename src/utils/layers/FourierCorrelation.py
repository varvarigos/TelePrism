...
class FourierBlock(nn.Module):
    ...
    def compl_mul1d(self, order, x, weights):
        ...
        if not torch.is_complex(x):
            x_flag = False
            x = torch.complex(x.float(), torch.zeros_like(x, dtype=torch.float))
        if not torch.is_complex(weights):
            w_flag = False
            weights = torch.complex(
                weights.float(), torch.zeros_like(weights, dtype=torch.float)
            )
        if x_flag or w_flag:
    ...
    def forward(self, q, k, v, mask):
        ...
        for wi, i in enumerate(self.index):
            ...
            out_ft[:, :, :, wi] = self.compl_mul1d(
                "bhi,hio->bho",
                x_ft[:, :, :, i],
                torch.complex(self.weights1.float(), self.weights2.float())[:, :, :, wi],
            )
        # Return to time domain
        x = torch.fft.irfft(out_ft, n=x.size(-1)).to(orig_dtype)
        return (x, None)
...
class FourierCrossAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask):
        # size = [B, L, H, E]
        B, L, H, E = q.shape
        xq = q.permute(0, 2, 3, 1).float()  # size = [B, H, E, L]
        xk = k.permute(0, 2, 3, 1).float()
        xv = v.permute(0, 2, 3, 1).float()
        orig_dtype = q.dtype
        ...
        xqkvw = self.compl_mul1d(
            "bhex,heox->bhox", xqkv_ft, torch.complex(self.weights1.float(), self.weights2.float())
        )
