...
class AutoCorrelation(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        # period-based dependencies (cast to float32: rfft doesn't support bfloat16)
        ...
        # time delay agg (time_delay_agg_* return float32; cast back to orig_dtype)
        if self.training:
            V = self.time_delay_agg_training(
                values.permute(0, 2, 3, 1).contiguous(), corr
            ).permute(0, 3, 1, 2).to(orig_dtype)
        else:
