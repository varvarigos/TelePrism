...
class NormalizationLayer(nn.Module):
    ...
    """
    def __init__(self, method: str, num_features: int):
        super().__init__()
        self.method = method
        if method.lower() == 'zscore':
            ...
            self.normalizer = Statistics_prediction(self.configs)
...
class RevIN(nn.Module):
    ...
    def _denormalize(self, x):
        if self.affine:
            # Reshape for broadcasting: [1, 1, C, 1]
            x = (x - self.affine_bias.view(1, 1, self.num_features, 1)) / (self.affine_weight.view(1, 1, self.num_features, 1) + self.eps**2)
