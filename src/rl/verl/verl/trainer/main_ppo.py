...
class TaskRunner:
    ...
    def run(self, config):
        ...
        # Use tokenizer_path if explicitly set, otherwise fall back to model path.
        _tok_path = config.actor_rollout_ref.model.get("tokenizer_path") or local_path
        if _tok_path != local_path:
            _tok_path = copy_to_local(_tok_path, use_shm=False)
        tokenizer = hf_tokenizer(_tok_path, trust_remote_code=trust_remote_code)
        # Used for multimodal LLM, could be None
        processor = hf_processor(_tok_path, trust_remote_code=trust_remote_code, use_fast=True)
