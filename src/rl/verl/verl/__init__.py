...
if modules:
    # Skip external module loading in vLLM worker subprocesses.
    # These spawned workers already have CUDA + vLLM loaded; importing
    # the external agent-loop modules triggers a cascading import chain
    # that crashes in that context.  The external modules (e.g. ts_agent_loop)
    # are only needed in AgentLoopWorker / main processes, not vLLM WorkerProcs.
    import sys as _sys
    _in_vllm_worker = any(k.startswith("vllm.v1.worker") or k.startswith("vllm.v1.executor") for k in _sys.modules)
    if not _in_vllm_worker:
